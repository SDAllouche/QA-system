    ...
    def processWithNLTK(self):
        ...
        def lemmatize(token, tag):
            ...
            if tag[0] in 'VN':
                ...
                return token
        self.tokens = [lemmatize(token, tag)
                       for token, tag in zip(self.tokens, self.pos_tags)]
        ...
        for word in categories[cat]:
            ...
            assert capital not in wordToCategory, \
                "%s is in both %s and %s" % (
                    capital, wordToCategory[capital], cat)
...
BASE_OBJECTS = ['Garfield', 'Pluto', 'Jon', 'Ahmed', 'Tom', 'Jerry']
...
def createBaseLanguageProcessor():
    ...
    categories = {
        'Noun': ['cat', 'tabby', 'mammal', 'person', 'animal', 'mouse', 'pet'],
        'Verb': ['likes', 'feeds'],
        'Name': BASE_OBJECTS,
    }
...
def createBaseEnglishGrammar():
    ...
    rules.append(GrammarRule(
        '$ROOT', ['$Statement'], lambda args: ('tell', args[0])))
    rules.append(GrammarRule(
        '$ROOT', ['$Question'], lambda args: ('ask', args[0])))
    rules.append(GrammarRule('$Statement', [
                 '$Clause', '.'], lambda args: args[0]))
    rules.append(GrammarRule(
        '$Question', ['$Clause', '?'], lambda args: args[0]))
...
def createToyGrammar():
    rules = []
    rules.append(GrammarRule('$S', ['the', '$B'],
                 lambda args: ('ask', args[0])))
...
def parseUtterance(utterance, rules, verbose=0):
    ...
    def applyRule(start, end, mid, rule, rhsIndex, children, score):
        ...
        if rhsIndex == len(rule.rhs):
            deriv = ParserDerivation(rule.sem(
                [child.form for child in children]), rule, children, score + rule.score)
            if verbose >= 3:
                print("applyRule: %s:%s%s %s += %s" %
        ...
        if isCat(a):  # Category
            for split in range(mid+1, end+1):
                ...
                for child in chart[mid][split].get(a, {}):
                    applyRule(start, end, split, rule, rhsIndex+1,
                              children + [child], score + child.score)
        else:  # Token
            if tokens[mid] == a:
                applyRule(start, end, mid+1, rule, rhsIndex+1, children, score)

    # Initialize
    # start => end => category => top K derivations (logical form, score)
    chart = [None] * n
    ...
    for start in range(0, n):
        ...
        if utterance.tokens[start] in ('be', 'not'):
            continue
        chart[start][start+1]['$'+utterance.pos_tags[start]
                              ].append(ParserDerivation(utterance.tokens[start], None, [], 0))
    ...
    for length in range(1, n+1):  # For each span length...
        for start in range(n - length + 1):  # For each starting position
            ...
            for cat in list(cell.keys()):
                cell[cat] = sorted(
                    cell[cat], key=lambda deriv: -deriv.score)[0:beamSize]
...
def trainGrammar(rules):
    ...
    examples.append(
        ('John is not happy.', ('tell', Not(Atom('Happy', 'john')))))
    examples.append(('John is a cat.', ('tell', Atom('Cat', 'john'))))
    examples.append(('John is not a dog.', ('tell', Not(Atom('Dog', 'john')))))
    examples.append(('John was born in Seattle.',
                    ('tell', Atom('Bear_in', 'john', 'seattle'))))
    examples.append(('John lives in Seattle.',
                    ('tell', Atom('Live_in', 'john', 'seattle'))))
    examples.append(('John lives in New York.',
                    ('tell', Atom('Live_in', 'john', 'new_york'))))
    examples.append(('John does not live in New York.',
                    ('tell', Not(Atom('Live_in', 'john', 'new_york')))))

    # Miscellaneous
    examples.append(('New York is a big city.', ('tell', And(
        Atom('Big', 'new_york'), Atom('City', 'new_york')))))
    examples.append(('If it is raining, it is wet.',
                    ('tell', Implies(Atom('Rain'), Atom('Wet')))))

    # Coordination
    examples.append(('John and Bill are cats.', ('tell', And(
        Atom('Cat', 'john'), Atom('Cat', 'bill')))))
    examples.append(('John is either happy or sad.',
                    ('tell', Xor(Atom('Happy', 'john'), Atom('Sad', 'john')))))
    examples.append(('John lives in Seattle or Portland.', ('tell', Or(
        Atom('Live_in', 'john', 'seattle'), Atom('Live_in', 'john', 'portland')))))
    examples.append(('Either it is raining or it is snowing.', ('tell', And(
        Or(Atom('Rain'), Atom('Snow')), Not(And(Atom('Rain'), Atom('Snow')))))))

    # Quantification
    examples.append(('Cats are animals.', ('tell', Forall(
        '$x1', Implies(Atom('Cat', '$x1'), Atom('Animal', '$x1'))))))
    examples.append(('A cat is an animal.', ('tell', Forall(
        '$x1', Implies(Atom('Cat', '$x1'), Atom('Animal', '$x1'))))))
    examples.append(('If a person lives in California, he is happy.', ('tell', Forall('$x1', Implies(
        And(Atom('Person', '$x1'), Atom('Live_in', '$x1', 'california')), Atom('Happy', '$x1'))))))
    examples.append(('John visited every city.', ('tell', Forall(
        '$x1', Implies(Atom('City', '$x1'), Atom('Visit', 'john', '$x1'))))))
    examples.append(('Every city was visited by John.', ('tell', Forall(
        '$x1', Implies(Atom('City', '$x1'), Atom('Visit_by', '$x1', 'john'))))))
    examples.append(('Every person likes some cat.', ('tell', Forall('$x1', Implies(Atom(
        'Person', '$x1'), Exists('$x2', And(Atom('Cat', '$x2'), Atom('Like', '$x1', '$x2'))))))))
    examples.append(('No city is perfect.', ('tell', Not(
        Exists('$x1', And(Atom('City', '$x1'), Atom('Perfect', '$x1')))))))

    examples.append(('Does John live in Sweden?',
                    ('ask', Atom('Live_in', 'john', 'sweden'))))
