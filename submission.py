...
def formula2c():
    ...
    # whether x has a child y
    def Child(x, y): return Atom('Child', x, y)
    def Daughter(x, y): return Atom(
        'Daughter', x, y)  # whether x has a daughter y
...
def formula2d():
    ...
    # whether x has a parent y
    def Parent(x, y): return Atom('Parent', x, y)
    def Grandmother(x, y): return Atom('Grandmother', x,
                                       y)  # whether x has a grandmother y
...
def liar():
    ...
    formulas.append(Implies(TellTruth(susan), CrashedServer(nicole)))
    # Fact 2: Mark: "No, it was Susan!"
    formulas.append(Implies(TellTruth(mark), CrashedServer(susan)))
    # Fact 3: Nicole: "Susan's a liar."
    formulas.append(Implies(TellTruth(nicole), Not(TellTruth(susan))))
    # Fact 4: Exactly one person is telling the truth.
    formulas.append(And(TellTruth(john), TellTruth(
        susan), TellTruth(nicole), TellTruth(mark)))
    # Fact 5: Exactly one person crashed the server.
    formulas.append(And(CrashedServer(john), CrashedServer(
        susan), CrashedServer(nicole), CrashedServer(mark)))
    # END_YOUR_CODE
...
def ints():
    ...
    def Successor(x, y): return Atom(
        'Successor', x, y)  # whether x's successor is y
    # whether x is larger than y
    ...
    # 0. Each number x has exactly one successor, which is not equal to x.
    formulas.append(Forall('$x', Exists(
        '$y', And(Successor('$x', '$y'), Not(Equals('$x', '$y'))))))
    ...
    # 2. The successor number of an even number is odd.
    formulas.append(Forall('$x', Implies(Even('$x'), Exists(
        '$y', And(Odd('$y'), Successor('$x', '$y'))))))

    # 3. The successor number of an odd number is even.
    formulas.append(Forall('$x', Implies(Odd('$x'), Exists(
        '$y', And(Even('$y'), Successor('$x', '$y'))))))
...
def createRule1():
    ...
    # return GrammarRule('$Clause', ['every', '$Noun', '$Verb', 'some', '$Noun'], lambda args: Forall('$x', Implies(Atom(args[1].title(), '$x'), Exists('$y', And(Atom(args[3].title(), '$y'), Atom(args[2].title(), '$x', '$y'))))))
    return GrammarRule('$Clause', ['every', '$Noun', '$Verb', 'some', '$Noun'], lambda args: Forall('$x', Forall('$y', Implies(And(Atom(args[0].title(), '$x'), Atom(args[1].title(), '$y')), Exists('$z', And(Atom(args[2].title(), '$y'), Atom(args[1].title(), args[0].lower(), '$y')))))))
    # END_YOUR_CODE
...
def createRule2():
    ...
    # BEGIN_YOUR_CODE (our solution is 3 lines of code, but don't worry if you deviate from this)
    # return GrammarRule('$Clause', ['there', 'is', 'some', '$Noun', 'that', 'every', '$Noun', '$Verb'], lambda args: Exists('$x', And(Atom(args[3].title(), '$x'), Forall('$y', Implies(Atom(args[5].title(), '$y'), Atom(args[2].title(), '$x', '$y'))))))
    return GrammarRule('$Clause', ['there', 'is', 'some', '$Noun', 'that', 'every', '$Noun', '$Verb'],
                       lambda args: Exists('$x', Forall('$y', Forall('$z', Implies(And(Atom(args[1].title(), '$y'), Atom(
                           args[2].title(), '$z')), Atom(args[0].title(), '$x'))))))
    # END_YOUR_CODE
...
def createRule3():
    ...
    return GrammarRule('$Clause', ['if', 'a', '$Noun', '$Verb', 'a', '$Noun', 'then', 'the', 'former', '$Verb', 'the', 'latter'],
                       lambda args: Forall('$x', Implies(And(Atom(args[0].title(), '$x'), Atom(args[1].title(), '$x', args[2].lower())), Atom(args[3].title(), '$x', args[2].lower()))))
    # END_YOUR_CODE
    # return GrammarRule('$Clause', ['if', 'a', '$Noun1', '$Verb', 'a', '$Noun2', 'then', 'the', 'former', '$Verb', 'the', 'latter'],lambda args: Forall('$x', Forall('$y', Implies(And(Atom(args[0].title(), '$x'), Atom(args[1].title(), '$y')), And(Atom(args[3].title(), '$x', '$y'), Atom(args[3].title(), '$y', '$x'))))))
